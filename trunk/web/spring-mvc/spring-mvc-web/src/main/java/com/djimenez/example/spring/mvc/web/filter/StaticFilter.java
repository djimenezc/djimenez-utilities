...
public class StaticFilter extends OncePerRequestFilter {
   ...
   * If it matches includes and not excludes, it forwards to the static resource
   * and ends the filter chain. Otherwise, it forwards to the next filter in the
   * chain.
   * 
   * @param request
   *          the current request
  ...
  public void doFilterInternal(final HttpServletRequest request,
    ...
    final boolean pathExcluded = PatternMatchUtils.simpleMatch(excludes, path);
    final boolean pathIncluded = PatternMatchUtils.simpleMatch(includes, path);
