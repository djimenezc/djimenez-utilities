package com.djimenez.example.spring.mvc.web.listener;
...
public class StartupListener implements ServletContextListener {
  ...
  public static void setupContext(final ServletContext context) {
    ...
    final LookupRoleManager mgr =
      (LookupRoleManager) ctx.getBean("lookupManager");

    // get list of possible roles
    context.setAttribute(ConstantsRole.AVAILABLE_ROLES, mgr.getAllRoles());
  ...
  public void contextInitialized(final ServletContextEvent event) {
    ...
    Map<String, Object> config =
      (HashMap<String, Object>) context.getAttribute(Constants.CONFIG);
    ...
    /*
     * String[] beans = ctx.getBeanDefinitionNames(); for (String bean : beans)
     * { log.debug(bean); }
     */
    ...
    try {
      final ProviderManager provider =
        (ProviderManager) ctx.getBean("_authenticationManager");
      for (final Object o : provider.getProviders()) {
        ...
        if (p instanceof RememberMeAuthenticationProvider) {
          ...
          if (ctx.getBean("passwordEncoder") != null) {
