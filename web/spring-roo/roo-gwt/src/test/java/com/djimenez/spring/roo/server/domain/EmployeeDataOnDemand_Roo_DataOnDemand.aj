...
privileged aspect EmployeeDataOnDemand_Roo_DataOnDemand {
    ...
    public Employee EmployeeDataOnDemand.getNewTransientEmployee(int index) {
        com.djimenez.spring.roo.server.domain.Employee obj = new com.djimenez.spring.roo.server.domain.Employee();
    ...
    public void EmployeeDataOnDemand.init() {
        ...
        data = com.djimenez.spring.roo.server.domain.Employee.findEmployeeEntries(0, 10);
        if (data == null) throw new IllegalStateException("Find entries implementation for 'Employee' illegally returned null");
        if (data.size() > 0) {
            return;
        }
        
        data = new java.util.ArrayList<com.djimenez.spring.roo.server.domain.Employee>();
        for (int i = 0; i < 10; i++) {
            com.djimenez.spring.roo.server.domain.Employee obj = getNewTransientEmployee(i);
            obj.persist();
            data.add(obj);
        }
    }
